        ...
        {user ? (
          <div className="flex items-center gap-4">
            <Link to="/profile" className="flex items-center gap-2 group">
              <div
                className={`w-9 h-9 rounded-full flex items-center justify-center transition-all ${activePage === "admin" ? "bg-red-100 group-hover:ring-2 group-hover:ring-red-300" : user.blocked ? "bg-red-100 ring-2 ring-red-300" : "bg-primary/10 group-hover:ring-2 group-hover:ring-primary/30"}`}
              >
                <span
                  className={`material-symbols-outlined text-lg ${activePage === "admin" || user.blocked ? "text-red-600" : "text-primary"}`}
                >
                  {activePage === "admin" ? "admin_panel_settings" : user.blocked ? "person_off" : "person"}
                </span>
              </div>
              <span
                className={`text-sm font-semibold hidden sm:inline transition-colors ${activePage === "admin" || user.blocked ? "text-red-600" : "text-on-surface group-hover:text-primary"}`}
              >
