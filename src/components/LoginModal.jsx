...
export default function LoginModal({ onClose, onSwitchToRegister, onLoginSuccess }) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (message.startsWith("__CHANGES_REQUESTED__|")) {
          const reason = message.replace("__CHANGES_REQUESTED__|", "");
          setChangesRequested({ email, password, reason });
          return;
        }
  ...
  const handleIdCardFile = (e) => {
    ...
    if (!file.type.startsWith("image/")) {
      setResubmitError("Please upload an image file");
      return;
    }
        ...
        >
          ...
          <div className="text-center py-4">
            ...
            <p className="text-sm text-on-surface-variant leading-relaxed mb-6">
              Your updated registration has been resubmitted for admin approval.
              The approval process may take up to 3 business days.
            </p>
            <button
              onClick={() => {
                setChangesRequested(null);
        ...
        >
          ...
          <div className="text-center mb-6">
            ...
            <p className="text-sm text-on-surface-variant">
              An admin reviewed your registration and requested changes.
            </p>
          </div>
          ...
          <p className="text-xs text-on-surface-variant mb-4">
            Update the fields below as needed. Leave fields empty to keep your original values.
          </p>

          <form onSubmit={handleResubmit} className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label htmlFor="resubmit-first-name" className="block text-sm font-semibold text-on-surface mb-1">
                  First Name
                </label>
                <input
                  id="resubmit-first-name"
                  name="resubmit-first-name"
                  type="text"
                  placeholder="Leave empty to keep"
                  className="w-full px-3 py-2.5 rounded-xl border border-outline-variant/30 bg-surface-container-lowest text-on-surface text-sm placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-primary/40 focus:border-primary transition-all"
                />
              </div>
              <div>
                <label htmlFor="resubmit-last-name" className="block text-sm font-semibold text-on-surface mb-1">
                  Last Name
                </label>
                <input
                  id="resubmit-last-name"
                  name="resubmit-last-name"
                  type="text"
                  placeholder="Leave empty to keep"
                  className="w-full px-3 py-2.5 rounded-xl border border-outline-variant/30 bg-surface-container-lowest text-on-surface text-sm placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-primary/40 focus:border-primary transition-all"
                />
            ...
            <div>
              <label htmlFor="resubmit-id-number" className="block text-sm font-semibold text-on-surface mb-1">
                ID Number
              </label>
              <input
                id="resubmit-id-number"
                name="resubmit-id-number"
                type="text"
                placeholder="Leave empty to keep"
                className="w-full px-3 py-2.5 rounded-xl border border-outline-variant/30 bg-surface-container-lowest text-on-surface text-sm placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-primary/40 focus:border-primary transition-all"
              />
              ...
              {newIdCardImage ? (
                <div className="relative rounded-xl overflow-hidden border border-outline-variant/30 group">
                  ...
                  <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-3">
                    ...
                    <button
                      type="button"
                      onClick={() => setNewIdCardImage(null)}
                      className="p-2 bg-white rounded-full shadow-lg hover:scale-110 transition-transform"
                    >
                ...
                <div
                  onClick={() => fileInputRef.current?.click()}
                  className="border-2 border-dashed border-outline-variant/30 rounded-xl flex flex-col items-center justify-center text-center cursor-pointer hover:border-primary/40 hover:bg-primary/[0.02] transition-all py-5 px-4"
                >
                  <span className="material-symbols-outlined text-primary/60 text-2xl mb-1">
                    badge
                  </span>
                  <p className="text-xs font-medium text-on-surface-variant">
                    Click to upload a new ID card image
                  </p>
                  <p className="text-[10px] text-on-surface-variant/70 mt-0.5">
                    Leave empty to keep your original image
                  </p>
                </div>
              )}
              <input
            ...
            <button
              ...
              className="w-full py-3 bg-primary text-white rounded-xl font-bold text-sm hover:shadow-lg hover:shadow-primary/20 transition-all disabled:opacity-60 disabled:cursor-not-allowed"
            >
          ...
          <button
            type="button"
            onClick={() => setChangesRequested(null)}
            className="w-full mt-3 py-2.5 text-on-surface-variant text-sm font-medium hover:text-on-surface transition-colors"
          >
            Back to Login
          </button>
        </div>
